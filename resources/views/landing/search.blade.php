...
@section('content')
    <div class="w-full mt-5 px-5 md:mx-auto md:max-w-2xl lg:max-w-4xl">
        <form action="{{ route('landing.search') }}">
            <x-text-input type="search" class="input-primary w-full" name="search" placeholder="Cari berdasarkan Judul Buku"
                value="" />
        </form>
        <div class="flex flex-wrap mt-5 px-2 justify-start">
            @if (count($data) > 0)
                ...
                <div class="w-full text-center">
                    <h1 class="text-2xl font-semibold text-gray-600">Buku tidak ditemukan</h1>
                </div>
            @endif
        </div>
    </div>
